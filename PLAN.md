...
### Фаза 14 — Чеклист выезда: hardcoded base + admin extras (завершено)
Триггер: один из гостей жаловался, что забытый невыключенный холодильник за следующие пару недель зацвёл плесенью. Универсальные правила («разморозить холодильник, дверь оставить открытой») должны быть в коде, а не зависеть от того, заполнил ли админ их через панель.

Гибридная архитектура: hardcoded base в i18n + admin-editable extras в Firestore.

- [x] **i18n** — `guide.checkout.baseItems` массив в трёх локалях (ru/en/de) с четырьмя универсальными пунктами:
  - Холодильник: выключить, разморозить, дверь открытой (плюс объяснение про плесень)
  - Закрыть окна и балконные двери
  - Вынести мусор
  - Проверить, не забыли ли вещи (зарядки, документы, продукты)
- [x] **`pages/guide/index.vue`** — `currentCheckoutItems` теперь объединяет `tm('guide.checkout.baseItems')` (база, всегда есть) + `getCheckoutItems(locale)` (квартиро-специфичные extras из Firestore). Чеклист рендерится **всегда** (раньше — только если admin что-то добавил).
- [x] **`pages/admin/guide/index.vue`** — заголовок изменён на «Дополнительные пункты чеклиста», добавлен hint, объясняющий что универсальные пункты уже встроены и здесь добавляются квартиро-специфичные.
- [x] **i18n adminGuide** — `checkoutExtrasTitle` и `checkoutExtrasHint` в трёх локалях.

**Что считается "квартиро-специфичным"** (через admin): отопление зимой на минимум, конкретное место для ключей, как разобраться с постельным бельём, посудомойка (если не у всех есть), и т.п.

**Не сделано (potential follow-ups):**
- Карточка-напоминание на `/apartment` за день до отъезда («не забудьте перед выездом — открыть гид»)
- Email-нотификация за день до отъезда с прямой ссылкой на чеклист
