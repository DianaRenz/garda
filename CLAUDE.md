...
**Invite system:** Registration is closed — only via one-time invite links (`/register/[token]`). Tokens are generated in `/admin/settings` using `composables/useInvite.ts` (`generateInvite(type)`, `validateToken`, `markTokenUsed`). `type: 'admin' | 'guest'` is stored in Firestore `invites` collection and returned by `validateToken`. Tokens expire in 7 days, marked `used: true` after registration. Admin invite → redirect `/admin`, guest invite → redirect `/apartment`. **Personal guest invites:** `generateGuestInvite(guestId)` creates a guest invite linked to a specific guest record (`guestId` field in `invites` doc). Generated from `/admin/guests` via invite button (shown only for unregistered guests). On registration: form is prefilled with guest data, `linkGuestToUser()` updates the guest record with `userId` and migrates all bookings (`guestId` match + `userId == null` → sets `userId`). `validateToken` returns `guestId` when present. If guest was deleted before registration, falls back to normal guest invite flow.
